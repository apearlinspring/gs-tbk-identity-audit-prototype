...
| 场景 | 地址 | 说明 |
| --- | --- | --- |
| 公网展示 | `https://gstbk.403edr.cn/` | 部署在公网服务器，适合远程面试或投递后给面试官预览。 |
...
3. 说明左侧“演示说明”：页面展示的是脱敏后的审计证据快照，不展示证书、私钥、节点配置或原始日志。
...
> 这个页面不是日志截图，而是从 E2E（End-to-End，端到端）运行后的 JSON（JavaScript Object Notation，数据交换格式）证据中聚合出来的审计视图。它只读、脱敏、不连接真实链节点，但能把用户、合约、交易、区块、查询判读和异常揭示放到一个页面里讲清楚。
