...
fn main() -> anyhow::Result<()> {
    ...
    ensure_pari_copy_is_executable(&pari_mirror)?;

    {
        let output = Command::new("sh")
            .arg(pari_mirror.join("Configure"))
...
#[cfg(unix)]
fn ensure_pari_copy_is_executable(pari_mirror: &Path) -> anyhow::Result<()> {
    let output = Command::new("chmod")
        .arg("-R")
        .arg("u+rwx")
        .arg(pari_mirror)
        .output()
        .context("chmod pari mirror")?;

    if !output.status.success() {
        std::io::stderr()
            .write_all(&output.stderr)
            .context("write chmod error to stderr")?;
        bail!("chmod pari mirror returned non-zero code");
    }
...
#[cfg(not(unix))]
fn ensure_pari_copy_is_executable(_pari_mirror: &Path) -> anyhow::Result<()> {
    Ok(())
}
