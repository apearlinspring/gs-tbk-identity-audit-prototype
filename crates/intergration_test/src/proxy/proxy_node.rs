...
use gs_tbk_scheme::messages::proxy::common_msg::{KeyGenMsg, KeyManageMsg, SetupMsg};
...
use gs_tbk_scheme::messages::node::key_manage_msg::{
    NodeToProxyKeyRecoverP2PMsg, NodeToProxyKeyRefreshOneP2PMsg,
};
use gs_tbk_scheme::messages::node::keygen_msg::{
    NodeToProxyKeyGenPhaseFiveP2PMsg, NodeToProxyKeyGenPhaseTwoP2PMsg,
};
use gs_tbk_scheme::messages::node::open_msg::{
...
use gs_tbk_scheme::messages::proxy::key_manage_msg::{
    ProxyToNodeKeyRefreshPhaseStartFlag, ProxyToNodeKeyRocoverPhaseStartFlag,
};
...
use gs_tbk_scheme::params::{
    DKGTag, JoinPhaseRevokeSharedMessageType, RevokePhaseSignSharedMessageType,
};
...
pub async fn main() -> Result<(), anyhow::Error> {
    ...
    let log_path = String::from(env::current_dir().unwrap().as_path().to_str().unwrap())
        + "/src/proxy/config/config_file/log4rs.yaml";
    log4rs::init_file(log_path, Default::default()).unwrap();

    // 初始化
    let gs_tbk_config_path = runtime_paths::proxy_config_path();
    ...
    // 创建setup阶段的一些共享变量
    let shared_node_setup_p2p_msg_vec =
        Arc::new(TokioMutex::new(Vec::<NodeToProxySetupPhaseP2PMsg>::new()));
    let setup_msg_num = Arc::new(TokioMutex::new(0));
    let setup_finish_num = Arc::new(TokioMutex::new(0));
    let shared_node_setup_finish_vec =
        Arc::new(TokioMutex::new(Vec::<NodeSetupPhaseFinishFlag>::new()));

    // 创建KeyGen阶段的共享变量
    let shared_keygen_phase_two_msg_A_vec = Arc::new(TokioMutex::new(Vec::<
        NodeToProxyKeyGenPhaseTwoP2PMsg,
    >::new()));
    let shared_keygen_phase_two_msg_B_vec = Arc::new(TokioMutex::new(Vec::<
        NodeToProxyKeyGenPhaseTwoP2PMsg,
    >::new()));
    let shared_keygen_phase_two_msg_C_vec = Arc::new(TokioMutex::new(Vec::<
        NodeToProxyKeyGenPhaseTwoP2PMsg,
    >::new()));
    let shared_keygen_phase_two_msg_O_vec = Arc::new(TokioMutex::new(Vec::<
        NodeToProxyKeyGenPhaseTwoP2PMsg,
    >::new()));
    let shared_keygen_phase_five_msg_vec = Arc::new(TokioMutex::new(Vec::<
        NodeToProxyKeyGenPhaseFiveP2PMsg,
    >::new()));

    // 创建Join阶段的共享变量
    let user_join_msg_num = Arc::new(TokioMutex::new(0));
    let mta_finish_msg_vec_map: HashMap<u16, Vec<NodeToProxyJoinIssuePhaseTwoMtAPhaseFinalP2PMsg>> =
        HashMap::new();
    let shared_mta_finish_msg_vec_map = Arc::new(TokioMutex::new(mta_finish_msg_vec_map));
    let join_phase_three_msg_vec_map: HashMap<u16, Vec<NodeToProxyJoinIssuePhaseThreeP2PMsg>> =
        HashMap::new();
    let shared_join_phase_three_msg_vec_map =
        Arc::new(TokioMutex::new(join_phase_three_msg_vec_map));
    let join_phase_four_msg_vec_map: HashMap<u16, Vec<NodeToProxyJoinIssuePhaseFourP2PMsg>> =
        HashMap::new();
    let shared_join_phase_four_msg_vec_map = Arc::new(TokioMutex::new(join_phase_four_msg_vec_map));
    let shared_join_shared_message = Arc::new(TokioMutex::new(NodeUserJoinRevokeSharedMessage {
        shared_message_map: HashMap::new(),
    }));

    // 创建Revoke阶段的共享变量
    let revoke_mta_finial_msg_map: HashMap<u16, Vec<NodeToProxyRevokePhaseTwoMtAPhaseFinalP2PMsg>> =
        HashMap::new();
    ...
    let shared_revoke_shared_message = Arc::new(TokioMutex::new(NodeUserRevokeSignSharedMessage {
        shared_message_map: HashMap::new(),
    }));

    // 创建open阶段的共享变量
    let open_phase_one_msg_vec_map: HashMap<u16, Vec<NodeToProxyOpenPhaseOneP2PMsg>> =
        HashMap::new();
    let shared_open_phase_one_msg_vec_map = Arc::new(TokioMutex::new(open_phase_one_msg_vec_map));
    let open_phase_two_msg_vec_map: HashMap<u16, Vec<NodeToProxyOpenPhaseTwoP2PMsg>> =
        HashMap::new();
    let shared_open_phase_two_msg_vec_map = Arc::new(TokioMutex::new(open_phase_two_msg_vec_map));

    // 创建KeyManage阶段的共享变量
    let shared_key_recover_msg_vec =
        Arc::new(TokioMutex::new(Vec::<NodeToProxyKeyRecoverP2PMsg>::new()));
    let shared_key_refresh_msg_vec =
        Arc::new(TokioMutex::new(Vec::<NodeToProxyKeyRefreshOneP2PMsg>::new()));
