...
use gs_tbk_scheme::messages::proxy::open_msg::{
...
impl Proxy {
    ...
    pub fn open_phase_one(
        &self,
        msg_vec: &Vec<NodeToProxyOpenPhaseOneP2PMsg>,
        ...
        // Merge
        let psi_1_gamma_O = msg_vec
            .iter()
            .fold(Point::<Bls12_381_1>::zero(), |acc, msg| {
                acc + &msg.psi_1_gamma_O_i
            });
    ...
    /// 合并出 A_gamma_C
    pub fn open_phase_two(
        &self,
        msg_vec: &Vec<NodeToProxyOpenPhaseTwoP2PMsg>,
    ) -> ProxyToNodesOpenPhaseTwoBroadcastMsg {
        assert_eq!(msg_vec.len(), self.threashold_param.share_counts as usize);
        let Aj_gamma_C = msg_vec
            .iter()
            .fold(Point::<Bls12_381_1>::zero(), |acc, msg| {
                acc + &msg.Aj_gamma_C_i
            });
