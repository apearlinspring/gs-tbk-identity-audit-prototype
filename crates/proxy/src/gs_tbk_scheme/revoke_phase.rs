...
use gs_tbk_scheme::messages::proxy::revoke_msg::{
    ...
    ProxyToUserRevokePhaseBroadcastMsg, RevokeInfo, TreeNodeBaseInfo,
};
use gs_tbk_scheme::messages::user::revoke_msg::RevokePhaseStartFlag;
use gs_tbk_scheme::params::{EiInfo, RevokeBBSSignature, RL};
...
impl Proxy {
    ...
    pub fn revoke_phase_one(&mut self, user_id: u16) -> ProxyToNodesRevokePhaseOneBroadcastMsg {
        ...
        for tree_node in self.tree.get_leaf_nodes() {
            let tree_node_time = Local
                .from_local_datetime(
                    &NaiveDateTime::parse_from_str(&tree_node.tau.realtime, "%Y-%m-%d %H:%M:%S")
                        .unwrap(),
                )
                .unwrap();
        ...
        self.revoke_info = Some(RevokeInfo {
        ...
        for id in ru_list {
            grt_map.insert(
                ...
                self.user_info_map
                    ...
                    .X_sim
                    .clone()
                    * self.revoke_info.as_ref().unwrap().yt.clone(),
            );
        }
    ...
    pub fn revoke_phase_two(
        &mut self,
        msg_vec: &Vec<NodeToProxyRevokePhaseTwoMtAPhaseFinalP2PMsg>,
    ) -> (
        ...
        for (tree_node_id, base_info) in leaf_node_base_info_map {
            ...
            let k: Scalar<Bls12_381_1> = msg_vec
                .iter()
                .map(|msg| msg.ki_pi_share_map.get(&tree_node_id).unwrap().ki.clone())
                .sum();

            let Bi = &base_info.base_j * (&exp_share.invert().unwrap() * &k);
            revoke_bbs_signatures_map.insert(
                tree_node_id.clone(),
                RevokeBBSSignature {
                    Bj: Bi,
                    xi_j: xi_j,
                    zeta_j: base_info.zeta_j.clone(),
                    vj: base_info.v_j.clone(),
                },
            );
        }
        let ei_info = EiInfo {
            t: self.revoke_info.as_ref().unwrap().t.clone(),
