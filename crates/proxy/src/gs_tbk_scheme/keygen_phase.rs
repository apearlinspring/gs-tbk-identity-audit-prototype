...
use gs_tbk_scheme::messages::node::keygen_msg::{
    NodeToProxyKeyGenPhaseFiveP2PMsg, NodeToProxyKeyGenPhaseTwoP2PMsg, ShareProof,
};
use gs_tbk_scheme::messages::{
    node::keygen_msg::ZkpProof,
    proxy::keygen_msg::{
        ProxyKeyGenPhaseOneBroadcastMsg, ProxyKeyGenPhaseStartFlag,
        ProxyToNodeKeyGenPhaseThreeP2PMsg, ProxyToNodesKeyGenPhasefiveBroadcastMsg,
    },
};
use gs_tbk_scheme::params::{
    ciphertext_to_hex, hex_to_ciphertext, hex_to_cldl_proof, hex_to_pk, Gpk,
};

impl Proxy {
    /// 生成部分公钥，随机选择参与方，然后广播给管理员
    pub fn keygen_phase_one(
        &mut self,
    ) -> (ProxyKeyGenPhaseStartFlag, ProxyKeyGenPhaseOneBroadcastMsg) {
        ...
        let participants: Vec<u16> = node_id_vec
            .choose_multiple(&mut rng, (self.threashold_param.threshold + 1) as usize)
            .cloned()
            .collect();
    ...
    pub fn keygen_phase_three(
        &self,
        msg_vec: Vec<NodeToProxyKeyGenPhaseTwoP2PMsg>,
    ) -> Result<HashMap<u16, ProxyToNodeKeyGenPhaseThreeP2PMsg>, Error> {
        ...
        let share_proof_map_vec: Vec<HashMap<u16, ShareProof>> = msg_vec
            .iter()
            .map(|msg| msg.share_proof_map.clone())
            .collect();
        let vss_commitments_vec: Vec<VerifiableSS<Bls12_381_1>> =
            msg_vec.iter().map(|msg| msg.vss_scheme.clone()).collect();
        for node in self.node_info_vec.as_ref().unwrap() {
            for i in 0..share_proof_map_vec.len() {
                ...
                let flag = share_proof
                    .verify(&group, &hex_to_pk(&node.pk_hex), &c_share, &commit)
                    .is_ok();
        ...
        if all_verify_flag {
            // Merge commitment
            let vss_commitments_vec: Vec<Vec<Point<Bls12_381_1>>> = msg_vec
                .iter()
                .map(|msg| msg.vss_scheme.commitments.clone())
                .collect();
            let total_vss_commitments = vss_commitments_vec.iter().fold(
                vec![Point::<Bls12_381_1>::zero(); vss_commitments_vec.len()],
                |acc, v| acc.iter().zip(v.iter()).map(|(a, b)| a + b).collect(),
            );
            ...
            for node in self.node_info_vec.as_ref().unwrap() {
                let (c_zero, _) = encrypt(
                    &group,
                    &hex_to_pk(&node.pk_hex),
                    &Scalar::<Bls12_381_1>::zero(),
                );
                let c_share_sum: Ciphertext = share_proof_map_vec.iter().fold(c_zero, |acc, v| {
                    eval_sum(
                        &acc,
                        &hex_to_ciphertext(&v.get(&node.id).as_ref().unwrap().c_share_hex.clone()),
                    )
                });
                msg_map.insert(
                    node.id.clone(),
                    ProxyToNodeKeyGenPhaseThreeP2PMsg {
                        dkgtag: dkgtag.clone(),
    ...
    /// 验证零知识证明，然后组合出完整的GPK
    pub fn keygen_phase_five(
        &mut self,
        msg_vec: &Vec<NodeToProxyKeyGenPhaseFiveP2PMsg>,
        ...
        for msg in msg_vec {
            ...
            let g_hat_z_gamma_A = &gpk.g_hat
                * Scalar::<Bls12_381_2>::from_bigint(&msg.zkp_proof.z_gamma_A_i.to_bigint());
            let g_hat_z_gamma_B = &gpk.g_hat
                * Scalar::<Bls12_381_2>::from_bigint(&msg.zkp_proof.z_gamma_B_i.to_bigint());
            let f_z_gamma_O = &gpk.f * &msg.zkp_proof.z_gamma_O_i;
            let flag = if ((g_z_gamma_A
                == &msg.zkp_proof.g_t + &msg.zkp_proof.g_gamma_A_i * &msg.zkp_proof.e
                && g_hat_z_gamma_A
                    == &msg.zkp_proof.g_hat_t
                        + &msg.vk_A_i
                            * Scalar::<Bls12_381_2>::from_bigint(&msg.zkp_proof.e.to_bigint()))
                && (g_z_gamma_B
                    == &msg.zkp_proof.g_t + &msg.zkp_proof.g_gamma_B_i * &msg.zkp_proof.e
                    && g_hat_z_gamma_B
                        == &msg.zkp_proof.g_hat_t
                            + &msg.vk_B_i
                                * Scalar::<Bls12_381_2>::from_bigint(&msg.zkp_proof.e.to_bigint()))
                && (g_z_gamma_O
                    == &msg.zkp_proof.g_t + &msg.zkp_proof.g_gamma_O_i * &msg.zkp_proof.e
                    && f_z_gamma_O == &msg.zkp_proof.f_t + &msg.g1_i * &msg.zkp_proof.e))
            {
                true
            } else {
                false
        ...
        if all_zkp_flag {
            let g_hat_gamma_A: Point<Bls12_381_2> =
                msg_vec.iter().map(|msg| msg.vk_A_i.clone()).sum();
            let g_hat_gamma_B: Point<Bls12_381_2> =
                msg_vec.iter().map(|msg| msg.vk_B_i.clone()).sum();
            ...
            Ok(ProxyToNodesKeyGenPhasefiveBroadcastMsg {
                sender: self.id,
                role: self.role.clone(),
                vk_A: g_hat_gamma_A,
                vk_B: g_hat_gamma_B,
