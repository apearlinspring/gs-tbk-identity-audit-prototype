...
use gs_tbk_scheme::messages::node::join_issue_msg::{
    NodeToProxyJoinIssuePhaseFourP2PMsg, NodeToProxyJoinIssuePhaseThreeP2PMsg,
    NodeToProxyJoinIssuePhaseTwoMtAPhaseFinalP2PMsg,
};
use gs_tbk_scheme::messages::proxy::join_issue_msg::{
    A1kCKInfo, ProxyToNodesJoinIssuePhaseFiveBroadcastMsg,
    ProxyToNodesJoinIssuePhaseFourBroadcastMsg, ProxyToNodesJoinIssuePhaseThreeBroadcastMsg,
    ProxyToNodesJoinIssuePhaseTwoBroadcastMsg, ProxyToUserJoinIssuePhaseOneP2PMsg,
    ProxyToUserJoinIssuePhaseThreeP2PMsg, TreeNodeBaseInfo, UserInfo,
};
use gs_tbk_scheme::params::{
    ciphertext_to_hex, cldl_proof_to_hex, hex_to_ciphertext, hex_to_cldl_proof, hex_to_pk,
};
use gs_tbk_scheme::params::{pk_to_hex, BBSSignature, Gsk, Reg};
use gs_tbk_scheme::tree::{Tree, TreeNode};
use gs_tbk_scheme::{
    messages::user::join_issue_msg::UserToProxyJoinIssuePhaseTwoP2PMsg, params::MTAParams,
};
...
impl Proxy {
    ...
    pub fn join_issue_phase_two(
        &mut self,
        msg: &UserToProxyJoinIssuePhaseTwoP2PMsg,
    ) -> Result<(ProxyToNodesJoinIssuePhaseTwoBroadcastMsg), Error> {
        ...
        if &msg.c_x == &vc_x {
            ...
            self.user_info_map
                .as_mut()
                .unwrap()
                .insert(id.clone() as u16, user_info.clone());

            Ok(ProxyToNodesJoinIssuePhaseTwoBroadcastMsg {
                sender: self.id,
                role: self.role.clone(),
    ...
    pub fn join_issue_phase_three(
        &mut self,
        msg_vec: &Vec<NodeToProxyJoinIssuePhaseTwoMtAPhaseFinalP2PMsg>,
    ) -> (
        ...
        assert_eq!(
            msg_vec.len(),
            (self.threashold_param.threshold + 1) as usize
        );
        ...
        // Merge
        let leaf_node_base_info_map = &self
            .user_info_map
            .as_ref()
            .unwrap()
            .get(&user_id)
            .unwrap()
            .leaf_node_base_info_map;
        ...
        for (tree_node_id, base_info) in leaf_node_base_info_map {
            ...
            let c_k = msg_vec.iter().fold(c_zero.clone(), |acc, msg| {
                eval_sum(
                    &acc,
                    &hex_to_ciphertext(&msg.c_ki_pi_share_map.get(&tree_node_id).unwrap().c_ki_hex),
                )
            });

            let A_1_k = &base_info.base_j * &exp_share.invert().unwrap();
            A_1_k_map.insert(tree_node_id.clone(), A_1_k.clone());
            A_1_k_c_k_map.insert(
                tree_node_id.clone(),
                A1kCKInfo {
                    A_1_k: A_1_k.clone(),
                    c_k_hex: ciphertext_to_hex(&c_k),
                    xi_j: xi_j,
                    zeta_j: base_info.zeta_j.clone(),
                    uj: base_info.u_j.clone(),
                    tau: self
                        .user_info_map
                        .as_ref()
                        .unwrap()
    ...
    /// 合并 A_gamma_C_k
    pub fn join_issue_phase_four(
        &self,
        msg_vec: &Vec<NodeToProxyJoinIssuePhaseThreeP2PMsg>,
        ...
        for tree_node_id in tree_node_id_vec {
            let A_gamma_C_k = msg_vec
                .iter()
                .fold(Point::<Bls12_381_1>::zero(), |acc, msg| {
                    acc + msg.A_gamma_C_i_k_map.get(&tree_node_id).unwrap()
                });
    ...
    pub fn join_issue_phase_five(
        &self,
        msg_vec: &Vec<NodeToProxyJoinIssuePhaseFourP2PMsg>,
        ...
        for tree_node_id in tree_node_id_vec {
            let A_gamma_C = msg_vec
                .iter()
                .fold(Point::<Bls12_381_1>::zero(), |acc, msg| {
                    acc + msg.A_gamma_C_k_ki_map.get(&tree_node_id).unwrap()
                });
