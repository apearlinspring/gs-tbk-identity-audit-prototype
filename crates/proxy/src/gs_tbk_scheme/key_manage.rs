...
impl Proxy {
    ...
    pub fn key_recover(
        &self,
        nodes_id: &Vec<u16>,
        ui_vec: Vec<Scalar<Bls12_381_1>>,
        xi_vec: Vec<Scalar<Bls12_381_1>>,
    ) -> bool {
    ...
    /// 恢复密钥（t,t）
    pub fn key_recover_add(
        &self,
        nodes_id: &Vec<u16>,
        ui_vec: Vec<Scalar<Bls12_381_1>>,
        xi_vec: Vec<Scalar<Bls12_381_1>>,
        addshare_vec: Vec<Scalar<Bls12_381_1>>,
    ) -> bool {
        let vss_scheme = VerifiableSS {
            parameters: ShamirSecretSharing {
                threshold: self.threashold_param.threshold,
    ...
    pub fn key_recover_phase(&self, msg_vec: &Vec<NodeToProxyKeyRecoverP2PMsg>) {
        ...
        let mut ui_vec_gamma_A: Vec<Scalar<Bls12_381_1>> = msg_vec
            .iter()
            .map(|msg| msg.keysinfo.gamma_A.ui.clone())
            .collect();
        ...
        let addshare_vec_gamma_A: Vec<Scalar<Bls12_381_1>> = msg_vec
            .iter()
            .map(|msg| msg.keysinfo.gamma_A.addshare.clone())
            .collect();
        if self.key_recover_add(
            &nodes_id,
            ui_vec_gamma_A,
            xi_vec_gamma_A,
            addshare_vec_gamma_A,
        ) {
            info!("Gamma A is vaild!")
        }

        let mut ui_vec_gamma_B: Vec<Scalar<Bls12_381_1>> = msg_vec
            .iter()
            .map(|msg| msg.keysinfo.gamma_B.ui.clone())
            .collect();
        let mut xi_vec_gamma_B: Vec<Scalar<Bls12_381_1>> = Vec::new();
        for id in participants.clone() {
            xi_vec_gamma_B.push(msg_map.get(&id).unwrap().keysinfo.gamma_B.xi.clone());
        }
        let addshare_vec_gamma_B: Vec<Scalar<Bls12_381_1>> = msg_vec
            .iter()
            .map(|msg| msg.keysinfo.gamma_B.addshare.clone())
            .collect();
        if self.key_recover_add(
            &nodes_id,
            ui_vec_gamma_B,
            xi_vec_gamma_B,
            addshare_vec_gamma_B,
        ) {
            info!("Gamma B is vaild!")
        }

        let mut ui_vec_gamma_C: Vec<Scalar<Bls12_381_1>> = msg_vec
            .iter()
            .map(|msg| msg.keysinfo.gamma_C.ui.clone())
            .collect();
        ...
        let mut ui_vec_gamma_O: Vec<Scalar<Bls12_381_1>> = msg_vec
            .iter()
            .map(|msg| msg.keysinfo.gamma_O.ui.clone())
            .collect();
    ...
    pub fn key_refresh_phase_two(
        &self,
        msg_vec: &Vec<NodeToProxyKeyRefreshOneP2PMsg>,
    ) -> Result<HashMap<u16, ProxyToNodeKeyRefreshPhaseTwoP2PMsg>, Error> {
        assert_eq!(msg_vec.len(), self.threashold_param.share_counts as usize);
        ...
        if all_verify_flag {
            ...
            for node in self.node_info_vec.as_ref().unwrap() {
                ...
                msg_map.insert(
                    node.id.clone(),
                    ProxyToNodeKeyRefreshPhaseTwoP2PMsg {
                        dkgtag: dkgtag.clone(),
