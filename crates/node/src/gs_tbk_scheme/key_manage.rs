...
use gs_tbk_scheme::messages::node::key_manage_msg::{
    KeyInfo, KeyInfoWithAddshare, KeysInfo, NodeToProxyKeyRecoverP2PMsg,
...
use gs_tbk_scheme::params::{
    ciphertext_to_hex, cldl_proof_to_hex, hex_to_ciphertext, hex_to_pk, pk_to_hex, DKGTag, Gpk,
};
...
impl Node {
    ...
    pub fn get_addshare(&mut self, dkgtag: &DKGTag) {
        ...
        match dkgtag {
            DKGTag::Gamma_A => {
                self.dkgparams.dkgparam_A.as_mut().unwrap().addshare = Some(
                    &li * self
                        .dkgparams
                        .dkgparam_A
                        .as_ref()
            ...
            DKGTag::Gamma_B => {
                self.dkgparams.dkgparam_B.as_mut().unwrap().addshare = Some(
                    &li * self
                        .dkgparams
                        .dkgparam_B
                        .as_ref()
            ...
            DKGTag::Gamma_O => {
                self.dkgparams.dkgparam_O.as_mut().unwrap().addshare = Some(
                    &li * self
                        .dkgparams
                        .dkgparam_O
                        .as_ref()
            ...
            DKGTag::Gamma_C => {
                self.dkgparams.dkgparam_C.as_mut().unwrap().addshare = Some(
                    &li * self
                        .dkgparams
                        .dkgparam_C
                        .as_ref()
    ...
    pub fn key_recover_phase(&self) -> NodeToProxyKeyRecoverP2PMsg {
        let (gamma_A_add, gamma_B_add) = if self
            .participants
            .as_ref()
            .unwrap()
            .contains(self.id.as_ref().unwrap())
        {
            (
                self.dkgparams
                    .dkgparam_A
    ...
    pub fn key_refresh_phase_three(
        &mut self,
        msg: ProxyToNodeKeyRefreshPhaseTwoP2PMsg,
    ) -> Result<(), Error> {
            ...
            .is_ok()
        {
            ...
            match dkgtag {
                DKGTag::Gamma_A => {
                    self.dkgparams.dkgparam_A.as_mut().unwrap().mskshare = Some(
                        self.dkgparams
                            .dkgparam_A
                            .as_ref()
                            .unwrap()
                ...
                DKGTag::Gamma_B => {
                    self.dkgparams.dkgparam_B.as_mut().unwrap().mskshare = Some(
                        self.dkgparams
                            .dkgparam_B
                            .as_ref()
                            .unwrap()
                ...
                DKGTag::Gamma_O => {
                    self.dkgparams.dkgparam_O.as_mut().unwrap().mskshare = Some(
                        self.dkgparams
                            .dkgparam_O
                            .as_ref()
                            .unwrap()
                ...
                DKGTag::Gamma_C => {
                    self.dkgparams.dkgparam_C.as_mut().unwrap().mskshare = Some(
                        self.dkgparams
                            .dkgparam_C
                            .as_ref()
                            .unwrap()
