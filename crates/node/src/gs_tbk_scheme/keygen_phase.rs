...
use gs_tbk_scheme::messages::node::keygen_msg::{
    ...
    NodeToProxyKeyGenPhaseTwoP2PMsg, ShareProof, ZkpProof,
};
use gs_tbk_scheme::messages::proxy::keygen_msg::{
    ProxyKeyGenPhaseOneBroadcastMsg, ProxyToNodeKeyGenPhaseThreeP2PMsg,
    ProxyToNodesKeyGenPhasefiveBroadcastMsg,
};
use gs_tbk_scheme::params::{
    ciphertext_to_hex, cldl_proof_to_hex, hex_to_ciphertext, hex_to_pk, DKGTag, Gpk,
};
...
impl Node {
    ...
    pub fn choose_dkgparam(&self, dkgtag: &DKGTag) -> &DKGParam {
        let dkgparam = match dkgtag {
            DKGTag::Gamma_A => self.dkgparams.dkgparam_A.as_ref().unwrap(),
            DKGTag::Gamma_B => self.dkgparams.dkgparam_B.as_ref().unwrap(),
            DKGTag::Gamma_O => self.dkgparams.dkgparam_O.as_ref().unwrap(),
            DKGTag::Gamma_C => self.dkgparams.dkgparam_C.as_ref().unwrap(),
        };
        dkgparam
    }
    ...
    pub fn keygen_phase_one(
        &mut self,
        dkgtag: DKGTag,
        msg: ProxyKeyGenPhaseOneBroadcastMsg,
    ...
    pub fn keygen_phase_two(
        &mut self,
        msg_vec: &Vec<NodeKeyGenPhaseOneBroadcastMsg>,
        ...
        let all_com_verify_tag = (0..msg_vec.len()).all(|i| {
            HashCommitment::<Sha256>::create_commitment_with_user_defined_randomness(
                &BigInt::from_bytes(msg_vec[i].yi.to_bytes(true).as_ref()),
                &msg_vec[i].blind_factor,
            ) == msg_vec[i].com
        });
        if all_com_verify_tag {
            ...
            let (vss_scheme, secret_shares) = VerifiableSS::share(
                self.threashold_param.threshold,
                self.threashold_param.share_counts,
                &dkgparam.ui.as_ref().unwrap(),
            );
            ...
            for node in self.node_info_vec.as_ref().unwrap() {
                ...
                let (c_share, share_proof) =
                    verifiably_encrypt(&self.group, &hex_to_pk(&node.pk_hex), (&share, &g_share));
            ...
            Ok(NodeToProxyKeyGenPhaseTwoP2PMsg {
                dkgtag: dkgtag,
                sender: self.id.unwrap(),
                role: self.role.clone(),
    ...
    /// 解密share，然后进行系数承诺验证
    pub fn keygen_phase_four(
        &mut self,
        msg: ProxyToNodeKeyGenPhaseThreeP2PMsg,
    ) -> Result<(), Error> {
        ...
        let x_i = decrypt(
            &group,
            &self.clkeys.sk,
            &hex_to_ciphertext(&msg.c_share_sum_hex),
        );
        // verify coefficient commitment
        if msg
            .vss_scheme_sum
            .validate_share(&x_i, self.id.unwrap())
            .is_ok()
        {
    ...
    pub fn keygen_phase_five(&self) -> NodeToProxyKeyGenPhaseFiveP2PMsg {
        ...
        let g_hat_gamma_A_i = &gpk.g_hat
            * Scalar::<Bls12_381_2>::from_bigint(
                &self
                    .dkgparams
                    .dkgparam_A
                    .as_ref()
