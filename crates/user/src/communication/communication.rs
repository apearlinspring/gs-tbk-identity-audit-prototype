...
pub fn join_issue_to_gstbk(
    msg_join: gs_tbk_scheme::messages::user::common_msg::JoinIssueMsg,
) -> String {
    let msg_str = serde_json::to_string(&gs_tbk_scheme::messages::common_msg::GSTBKMsg::GSTBKMsgU(
        gs_tbk_scheme::messages::user::common_msg::GSTBKMsg::JoinIssueMsg(msg_join),
    ))
    .unwrap();
    return msg_str;
}
...
pub fn sign_to_gstbk(msg_sign: gs_tbk_scheme::messages::user::common_msg::SignMsg) -> String {
    let msg_str = serde_json::to_string(&gs_tbk_scheme::messages::common_msg::GSTBKMsg::GSTBKMsgU(
        gs_tbk_scheme::messages::user::common_msg::GSTBKMsg::SignMsg(msg_sign),
    ))
    .unwrap();
    return msg_str;
}
